...
- **Full region-graph access rules.** Mirror the apworld's per-level Lua-style logic (jump, dive, explosives, jetpack fuel, glitch tricks, region-level key gates the v0.2 heuristic missed). UT uses the real region graph; our tracker currently uses a simplified `level-unlock + per-name-heuristic key` model. Implementation: AST-walk each level's `main_region()`, build the region DAG, translate `r.jump`/`r.can_open`/`r.explosives`/`r.jetpack(N)`/etc. into PopTracker access_rules, with complementary `ab_unlocked`/`int_unlocked` toggles to handle the YAML `unlock_abilities`/`unlock_interact` cases (PopTracker access_rules don't natively support negation).
